...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled( frame_bb.Min, grab_bb.Max, GetColorU32( ImGuiCol_CheckMark ), style.FrameRounding ); // Main gradient.
    window->DrawList->AddRectFilledMultiColor( frame_bb.Min, grab_bb.Max, GetColorU32( ImVec4( 0.0f, 0.0f, 0.0f, 0.05f ) ), GetColorU32( ImVec4( 0.0f, 0.0f, 0.0f, 0.05f ) ), GetColorU32( ImVec4( 0.0f, 0.0f, 0.0f, 0.38f ) ), GetColorU32( ImVec4( 0.0f, 0.0f, 0.0f, 0.38f ) ) ); // Main gradient.
    ...
    if ( hovered ) {
        RenderText( ImVec2( frame_bb.Max.x - ImGui::CalcTextSize( value_buf ).x + 20, frame_bb.Min.y - 7 ), value_buf, value_buf_end );
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if ( !( *v ) )
        window->DrawList->AddRect( check_bb.Min - ImVec2( 1.f, 1.f ), check_bb.Max + ImVec2( 1.f, 1.f ), GetColorU32( ImGuiCol_CheckMark, it->second ), style.FrameRounding ); // anim

    window->DrawList->AddRectFilled( check_bb.Min + ImVec2( pad, pad ), check_bb.Max - ImVec2( pad, pad ), GetColorU32( ImGuiCol_CheckMark, it_press->second.alpha_anim ), style.FrameRounding ); // fill
    if ( *v ) {
